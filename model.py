...
def PredictWithData(data,data_yuan,name,model,modelname,INPUT_DIMS = 13,TIME_STEPS = 20):
    ...
    print("data_yuan shape", data_yuan.shape)
    ## we won't include the error terms from ARIMA for XGboost model
...
def xgb_scheduler(data,y_hat):
    ...
    train, test = prepare_data(data, n_test=len(y_hat), n_in=8, n_out=1)
    #test = test.drop(test.columns[[-1]], axis=1)
    #test = test.append(pd.DataFrame(y_hat))
    print("y_hat", y_hat)
