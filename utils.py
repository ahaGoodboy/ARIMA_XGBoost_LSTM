...
def prepare_data(series, n_test, n_in, n_out):
    ...
    supervised_data.to_csv('supervised.csv')
    train, test = supervised_data.loc[:(len(supervised_data) - n_test + n_in - 1), :], supervised_data.loc[(len(supervised_data) - n_test + n_in):, :]
    return train, test